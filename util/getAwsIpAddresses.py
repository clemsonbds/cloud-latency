...
try:
    if nodeType == "bastion":
        ...
        response = subprocess.check_output(['aws', 'ec2', '--region', str(region), 'describe-instances', '--filters', 'Name=tag:Name,Values=' + str(expName)+'-Instance ', 'Name=instance-state-name,Values=running'], stderr=subprocess.STDOUT)
        temp = json.loads(response)
